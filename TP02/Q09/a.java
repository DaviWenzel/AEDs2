...
public class Q09 {
    public static void gerarLog(long inicio, long fim, int comparacoes) {
	    ...
	    String nomelog = "matricula_heapsort.txt";
  ...
  public static void heapSort(int[] ids, int length) {
    long inicio = System.nanoTime();
    int comparacoes = 0;
    for (int i = length / 2 - 1; i >= 0; i--) {
        comparacoes += heapify(ids, length, i);
    }
    for (int i = length - 1; i > 0; i--) {
        ...
        comparacoes += heapify(ids, i, 0);
    }

    long fim = System.nanoTime();
    gerarLog(inicio, fim, comparacoes);
}
public static int heapify(int[] ids, int n, int i) {
    int maior = i;
    int esq = 2 * i + 1;
    int dir = 2 * i + 2;
    int comparacoes = 0;
    ...
    if (esq < n) {
        comparacoes++;
        if (compareShows(ids[esq], ids[maior]) > 0) {
            maior = esq;
        }
    }
    ...
    if (dir < n) {
        comparacoes++;
        if (compareShows(ids[dir], ids[maior]) > 0) {
            maior = dir;
        }
    }
    ...
    if (maior != i) {
        ...
        comparacoes += heapify(ids, n, maior);
    }
...
public static int compareShows(int a, int b) {
    int cmp = show[a].getDirector().compareTo(show[b].getDirector());
    if (cmp != 0) return cmp;
