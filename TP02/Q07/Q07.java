...
public class Q07 {
    public static void gerarLog(long inicio, long fim, int comparacoes) {
	    ...
	    String nomelog = "matricula_insercao.txt";
  ...
  public static void insertionSort(int[] ids, int length) {
    ...
    for (int i = 1; i < length; i++) {
        ...
        while (j >= 0) {
            comparacoes++;
            int cmp = show[ids[j]].getType().compareTo(show[a].getType());
            if (cmp > 0 || (cmp == 0 && show[ids[j]].getTitle().compareTo(show[a].getTitle()) > 0)) {
                ids[j + 1] = ids[j];
                j--;
            } else {
                break;
