...
int encontrarMenorTitulo(DisneyPlus *arr, int inicio, int n) {
  int menor = inicio;
  for (int i = inicio + 1; i < n; i++) {
    if (strcasecmp(arr[i].title, arr[menor].title) < 0) {
      menor = i;
    }
  }
  return menor;
...
void selectionSortPorTitulo(DisneyPlus *arr, int n, int indice) {
  if (indice >= n - 1) {
    return;
  }

  int menorIndice = encontrarMenorTitulo(arr, indice, n);

  if (menorIndice != indice) {
    DisneyPlus temp = arr[indice];
    arr[indice] = arr[menorIndice];
    arr[menorIndice] = temp;
  }

    selectionSortPorTitulo(arr, n, indice + 1);
}
...
int main() {
  ...
  selectionSortPorTitulo(registrosSelecionados, totalSelecionados, 0);
