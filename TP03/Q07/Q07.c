...
void enfileirar(Fila* fila, Show* show) {
    if (fila->tamanho == 5) {
...
void enfileirarComLimite(Fila* f, Show* s) {
    if (f->tamanho >= 5) {
        desenfileirar(f);
    }
    ...
    int soma = 0;
    int qtd = 0;
    for (Celula* c = f->primeiro; c != NULL; c = c->prox) {
...
int main() {
    ...
    while (fgets(inputId, sizeof(inputId), stdin)) {
        ...
        for (int i = 0; i < count; i++) {
            if (strcmp(shows[i]->id, inputId) == 0) {
                enfileirarComLimite(&fila, shows[i]);
                encontrado = 1;
                break;
            }
        }
    ...
    int n;
    if (scanf("%d", &n) != 1) {
        fprintf(stderr, "Erro ao ler quantidade de comandos.\n");
        return 1;
    }
    getchar(); 
        ...
        if (comando[0] == 'I') {
            ...
            for (int j = 0; j < count; j++) {
                if (strcmp(shows[j]->id, id) == 0) {
                    enfileirarComLimite(&fila, shows[j]);
                    encontrado = 1;
                    break;
                }
            }
            if (!encontrado) {
                fprintf(stderr, "ID não encontrado para inserção: %s\n", id);
            }
        } else if (comando[0] == 'R') {
            Show* s = desenfileirar(&fila);
            if (s != NULL) {
                printf("(R) %s\n", s->title);
            } else {
                fprintf(stderr, "Fila vazia ao tentar remover.\n");
            }
        }
    }
